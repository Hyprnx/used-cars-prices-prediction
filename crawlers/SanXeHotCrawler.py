...
class SanXeHotCrawler(BaseClass):
    def __init__(self):
        super().__init__()
        file_path = path + '/sanxehot_links.json'
        self.df = pd.read_json(file_path).T
    ...
    def crawl(self):
        for i in range(0, len(self.df)):
            ...
            req = self.requests_session.get(url, headers=HEADERS).text
            soup = BeautifulSoup(req, 'lxml')
            ...
            car_info = pd.Series({'name': name, 'brand': None, 'source_url': url, 'type': type,
            ...
            self.car_df = self.car_df.append(car_info, ignore_index=True)
